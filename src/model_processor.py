...
class ModelProcessor:
    ...
    def _recursive_transform(self, expr):
        ...
        if isinstance(expr, relax.Function):
            # 对于Function类型，使用with_body方法
            updated_body = self._recursive_transform(expr.body)
            return expr.with_body(updated_body)
        elif isinstance(expr, relax.SeqExpr):
            # 对于SeqExpr类型，处理其body属性
            updated_seq = []
            for e in expr.body:
                updated_seq.append(self._recursive_transform(e))
            # 创建新的SeqExpr对象
            return relax.SeqExpr(updated_seq, expr.result)
        elif hasattr(expr, "then_branch"):
            expr = expr.with_then_branch(self._recursive_transform(expr.then_branch))
        elif hasattr(expr, "else_branch"):
            expr = expr.with_else_branch(self._recursive_transform(expr.else_branch))
        elif hasattr(expr, "args") and isinstance(expr.args, list):
            new_args = []
